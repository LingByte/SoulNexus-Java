...
import com.lingecho.common.core.storage.FileStorageAdapter;
...
public class WordCloudUtil {
    ...
    public WordCloudUtil(FileStorageAdapter fileStorageAdapter) {
     ...
     * @param texts    文本
     */
    public String generate(String artistId, List<String> texts) {
        ...
        if (num.equals(1)) {
            ...
            try {
                ...
                return fileStorageAdapter.uploadFile(file, filename);
            } catch (Exception e) {
                log.info("WordCloud Generate Failed ...");
                e.printStackTrace();
            }
        } else {
            ...
            try {
                // 随机选择背景图片
                String[] backgroundImages = {"/background/sm-logo.png", "/background/whale_small.png"};
                Random imgRandom = new Random();
                String selectedImage = backgroundImages[imgRandom.nextInt(backgroundImages.length)];

                wordCloud.setBackground(new PixelBoundryBackground(
                        Objects.requireNonNull(getClass().getResourceAsStream(selectedImage))));
                // 设置颜色调色板
                wordCloud.setColorPalette(new ColorPalette(randomColor));
